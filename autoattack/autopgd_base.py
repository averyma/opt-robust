...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    ...
                    if self.loss == 'bce':
                        loss_indiv = criterion_indiv(logits, y.float().view(-1,1))
                    else:
        ...
        for i in range(self.n_iter):
            ...
            grad /= float(self.eot_iter)
            if self.loss == 'bce':
                pred = (logits.detach().squeeze()>0) == y
            else:
            ...
            with torch.no_grad():
              ...
              if self.loss == 'bce':
                loss_best[ind] = y1[ind].unsqueeze(2) + 0
                loss_best_steps[i + 1] = loss_best.squeeze() + 0
              else:
