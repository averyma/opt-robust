...
class AutoAttack():
    ...
    def run_standard_evaluation_return_robust_accuracy(self, x_orig, y_orig, bs=250, return_labels=False, return_perturb=False):
        if self.verbose:
            print('using {} version including {}'.format(self.version,
                ', '.join(self.attacks_to_run)))
    ...
    def run_standard_evaluation_return_robust_accuracy_binary(self, x_orig, y_orig, bs=250, return_labels=False, return_perturb=False):
        if self.verbose:
            print('using {} version including {}'.format(self.version,
                ', '.join(self.attacks_to_run)))
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(n_batches):
                ...
                y = y_orig[start_idx:end_idx].clone().to(self.device)
#                 output = self.get_logits(x).max(dim=1)[1]
                output = (self.get_logits(x)>0).squeeze()
                y_adv[start_idx: end_idx] = output
                correct_batch = y.eq(output)
                robust_flags[start_idx:end_idx] = correct_batch.detach().to(robust_flags.device)
...
#                     ipdb.set_trace()
#                     output = self.get_logits(adv_curr).max(dim=1)[1]
                    output = (self.get_logits(adv_curr)>0).squeeze().long()
                    false_batch = ~y.eq(output).to(robust_flags.device)
                    non_robust_lin_idcs = batch_datapoint_idcs[false_batch]
                    robust_flags[non_robust_lin_idcs] = False
